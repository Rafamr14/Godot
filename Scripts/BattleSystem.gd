...
var turn_queue: Array = []  # CORREGIDO: Array genérico
...
func start_battle(player_team: Array, enemy_team: Array):  # CORREGIDO: Arrays genéricos
...
func _setup_battle_queue(player_team: Array, enemy_team: Array):  # CORREGIDO: Arrays genéricos
	turn_queue.clear()
	var all_characters: Array = []  # CORREGIDO: Array genérico
	all_characters.append_array(player_team)
	all_characters.append_array(enemy_team)
	
	# Resetear Combat Readiness
	for character in all_characters:
		if character is Character:  # Verificación de tipo
...
func _get_next_character() -> Character:
	...
	for character in turn_queue:
		if character is Character and character.is_alive() and character.combat_readiness >= 100.0:
...
func _advance_all_cr():
	...
	for character in turn_queue:
		if character is Character and character.is_alive():
...
func _get_ai_targets(caster: Character, skill: Skill) -> Array:  # CORREGIDO: Array genérico
	var targets: Array = []  # CORREGIDO: Array genérico
	...
	match skill.target_type:
		Skill.TargetType.SINGLE_ENEMY:
			var enemies = game_manager.player_team.filter(func(char): return char is Character and char.is_alive())
		...
		Skill.TargetType.ALL_ENEMIES:
			targets = game_manager.player_team.filter(func(char): return char is Character and char.is_alive())
		
		Skill.TargetType.SINGLE_ALLY:
			var allies = game_manager.enemy_team.filter(func(char): return char is Character and char.is_alive())
		...
		Skill.TargetType.ALL_ALLIES:
			targets = game_manager.enemy_team.filter(func(char): return char is Character and char.is_alive())
...
func execute_skill(caster: Character, skill: Skill, targets: Array):  # CORREGIDO: Array genérico
...
func _check_battle_end() -> bool:
	...
	var player_alive = game_manager.player_team.any(func(char): return char is Character and char.is_alive())
	var enemy_alive = game_manager.enemy_team.any(func(char): return char is Character and char.is_alive())
...
func create_test_enemies() -> Array:  # CORREGIDO: Array genérico
	var enemies: Array = []  # CORREGIDO: Array genérico
